...
USER = {"id": "maya", "name": "Maya Chen", "role": "employee", "location": "San Francisco", "device": "MacBook", "scope": "sf", "external_account": None}
...
async def users(isolated_db, monkeypatch):
    ...
    for user in [USER, OTHER, OPERATOR]:
        await query("INSERT INTO users(id,name,role,location,device,scope,external_account) VALUES($1,$2,$3,$4,$5,$6,$7)", list(user.values()))
...
async def operation(report_id, kind="create"):
    rows = (await query("SELECT * FROM connector_operations WHERE report_id=$1 AND kind=$2 ORDER BY created_at", [report_id, kind])).rows
...
def source(kind, service, **extra):
    ...
    return {"id": str(uuid4()), "kind": kind, "title": "Approved help", "body": "Reconnect Wi-Fi safely.", "service": service, "visibility": "all", "location": "San Francisco", "status": "approved", "metadata": {}, "created_at": now, "updated_at": now, **extra}
...
async def test_one_clarification_then_general_intake():
    ...
    await intake({"reportId": report["id"], "text": "Still no luck", "submissionKey": str(uuid4())}, USER)
    await process_intake(report["id"], USER, {"retrieve": no_sources})
    report = await get_report(report["id"], USER)
    assert report["clarifications"] == 1
    assert report["state"] == "submission_pending"
...
async def test_unknown_creates_never_blindly_replay(failure):
    report = await create()
    op = await operation(report["id"])
    await query("UPDATE connector_operations SET state='unknown',attempts=$2 WHERE id=$1", [op["id"], 5 if failure == "exhausted" else 1])
    ...
    assert (await operation(report["id"]))["state"] == ("failed" if failure in {"multiple", "exhausted"} else "unknown")
...
async def test_failed_routing_update_preserves_created_request():
    ...
    assert (await query("SELECT count(*) n FROM connector_operations WHERE report_id=$1 AND kind='create'", [report["id"]])).rows[0]["n"] == 1
...
async def test_security_support_bypass_stays_restricted():
    report = await create("I cannot get in")
    await intake({"reportId": report["id"], "action": "support", "text": "My account was hacked", "submissionKey": str(uuid4())}, USER)
    saved = await get_report(report["id"], USER)
    assert saved["decision"]["team"] == "Security Review"
    assert saved["state"] == "operator_review"
    op = await operation(report["id"])
    await process_operation(op["id"])
    assert (await operation(report["id"]))["state"] == "failed"
...
async def test_related_follow_rechecks_visibility_and_open_status():
    incident = source("incident", "atlas", status="open", visibility="sf", metadata={"curated": True})
    await query("INSERT INTO sources(id,kind,title,body,service,visibility,location,status,metadata) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)", [incident[k] for k in ["id", "kind", "title", "body", "service", "visibility", "location", "status", "metadata"]])
    ...
    with pytest.raises(ValueError, match="no longer available"):
        await intake({"reportId": report["id"], "action": "follow", "submissionKey": str(uuid4())}, USER)
...
async def test_review_timer_deduplicates_and_respects_mode(monkeypatch):
    report = await create()
    await query("UPDATE reports SET created_at=now()-interval '20 minutes' WHERE id=$1", [report["id"]])
    monkeypatch.setenv("APP_MODE", "live")
    await review_timers()
    assert (await query("SELECT count(*) n FROM operator_events")).rows[0]["n"] == 0
    monkeypatch.setenv("APP_MODE", "demo")
    await review_timers()
    await review_timers()
    assert (await query("SELECT count(*) n FROM operator_events")).rows[0]["n"] == 1
...
async def test_provider_resolution_and_reassignment_sync_without_rerouting():
    ...
    report = await get_report(report["id"], USER)
    await query("UPDATE demo_tickets SET team='Endpoint',status='Resolved' WHERE key=$1", [report["provider_key"]])
    await query("UPDATE reports SET synced_at=now()-interval '2 minutes' WHERE id=$1", [report["id"]])
...
async def test_live_support_runs_extraction_preserves_evidence_and_skips_tried_procedure(monkeypatch):
    monkeypatch.setenv("APP_MODE", "live")
    ...
    article = source("article", "laptop", body="Reconnect the monitor cable.", metadata={"procedure": "monitor"})
    async def retrieve(text, user):
        return [article]
    async def extract(text, message_ids, procedure):
        assert procedure["id"] == article["id"]
        return {"usage": {"input": 30, "output": 40}, "data": {
            "summary": "External monitor flickers after reconnecting cables",
            "service": "laptop", "serviceQuote": "external monitor", "symptomQuote": "My external monitor flickers.",
            "impactQuote": "Only I am affected.", "urgencyQuote": None, "deviceQuote": "external monitor",
            "startedQuote": None, "workaroundQuote": None, "attemptedStepsQuotes": ["I reconnected the cables."],
            "supportRequestQuote": "Please send this to IT support.", "procedureAttemptedQuote": "I reconnected the cables.",
            "securityQuote": None, "evidenceIds": message_ids,
        }}
    ...
    context = (await query("SELECT context FROM context_snapshots WHERE report_id=$1", [id])).rows[0]["context"]
    assert context["directory"] is None and context["simulated"] is False
    assert (await operation(id))["payload"]["summary"] == report["summary"]
...
async def test_live_model_failure_preserves_original_message_and_restricted_security(monkeypatch):
    ...
    for text, team, state in [("VPN cannot connect", "Service Desk", "submission_pending"),
        ...
        assert (await query("SELECT body FROM messages WHERE report_id=$1 AND role='user'", [id])).rows[0]["body"] == text
...
@pytest.mark.parametrize("failure,expected", [("validation", "failed"), ("rejected", "failed"), ("rate-limit", "pending"), ("unexpected-create", "unknown")])
async def test_create_error_boundary_distinguishes_safe_rejection_from_uncertainty(failure, expected):
    report = await create()
    op = await operation(report["id"])
